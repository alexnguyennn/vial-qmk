...
- 2026-07-12: Daemon RPC broker added. HID conflict between the state daemon and vial-qs.py (both want exclusive raw-HID access) resolved by making the daemon the sole HID owner and adding a local Unix-socket RPC (`/tmp/qmk-state-daemon.sock`). New subcommands: `qmk-state-daemon qsid list|get|set`, `qmk-state-daemon ping`. QSID RPCs are serialized (one in flight) and share the same HID transport as the push state pipeline. Verified live against the connected board: qsid list shows QSIDs 28 (flow_tap_shift_delta) and 29 (flow_tap_shift_min_clamp), get/set/read-back round-trips reliably, and sketchybar state updates continue in parallel.
